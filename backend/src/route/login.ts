        ...
        app
            ...
            .post(
                ...
                async ({
                    ...
                    try {
                        let authorizeAnswer = await user.doLogin(
                            email,
                            passwordHash,
                        );
                        if (authorizeAnswer) {
            ...
            .post(
                "register",
                async ({ user, body: { name, email, passwordHash } }) => {
                    try {
                        const newUser = await user.newPasswordUser(
                            name,
                            email,
                            passwordHash,
                        );
            ...
            .post(
                "reset-password",
                async ({ user, body, cookie: { auth } }) => {
                    try {
                        const resetResult = await user.resetPassword(
                            body.tokenId,
                            body.email,
