        ...
        try {
            ...
            if (type == "email_verification") {
                templatePath = path.join(
                    __dirname,
                    "email-template/token/email_verification.html",
                );
                emailSubject = "EnvXSync Email Verification";
            } else if (type == "password_reset") {
                templatePath = path.join(
                    __dirname,
                    "email-template/token/password_reset.html",
                );
    ...
    async verifyToken(
        ...
        try {
            ...
            const existingToken = (await db
                ...
                .from(tokenTable)
                .where(
                    sql`${tokenTable.user_id} = ${user_id} and ${tokenTable.token} = ${token} and ${tokenTable.type} = ${type} and ${tokenTable.used} = false`,
                )
                .then((res) => res[0])) as Token;
            if (!existingToken) {
